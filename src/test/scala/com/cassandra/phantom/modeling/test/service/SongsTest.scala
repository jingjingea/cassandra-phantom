...
/**
  ...
  */
class SongsTest extends CassandraSpec with ProductionDatabase with Connector.connector.Connector {

  /** show
    * When extending your database in this case [[ProductionDatabase]]
  ...
  it should "find a song by id" in {
    ...
    whenReady(chain) { (res: Option[Song]) =>
